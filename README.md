...
产物保存到 `output/{视频ID}/`：`video.mp4` · `audio.mp3` · `transcript_{识别语言}.md`（如 `transcript_auto.md`、`transcript_zh-CN.md`；语言功能前的旧文件 `transcript.md` 在默认识别时仍可命中）。`output/.url_index.json` 记录链接→视频映射，重复链接提取零网络解析。
